...
models.Base.metadata.create_all(bind=engine) # サーバ内にDBを作成
...
@app.get("/", status_code=status.HTTP_200_OK) # 関数定義、うまくいけば200を返す
async def read_all(db: db_dependency): # DBを入力
    return db.query(Todos).all() # db.query(Todos).all()で全件取得


@app.get("/todo/{todo_id}", status_code=status.HTTP_200_OK) # 関数定義、うまくいけば200を返す
...
@app.post("/todo", status_code=status.HTTP_201_CREATED) # 関数定義、うまくいけば201を返す
...
@app.put("/todo/{todo_id}", status_code= status.HTTP_204_NO_CONTENT) # 関数定義、うまくいけば204を返す
...
@app.delete("/todo/{todo_id}", status_code=status.HTTP_204_NO_CONTENT) # 関数定義、うまくいけば204を返す
